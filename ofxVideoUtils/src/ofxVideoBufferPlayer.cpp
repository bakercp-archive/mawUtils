...
void ofxVideoBufferPlayer::update() {
    ...
    if(shouldResize) {
        ...
        //cout << "RESIZING WAS: " << buffer.size() << " newSize=" << newSize << endl;
        
        //if(currentFrame >= newSize) {
            currentFrame = 0;//newSize-1; // pull it back
        //}
        ofxVideoBuffer::resize(newSize);
        
        //cout << "NEW SIZE IS: " << buffer.size() << endl;
...
//--------------------------------------------------------------
void ofxVideoBufferPlayer::resize(int _newSize) {
    shouldResize = true;
    newSize = _newSize;
  //  cout << "NEW SIZE::: " << newSize << "-" << _newSize << endl;
}
