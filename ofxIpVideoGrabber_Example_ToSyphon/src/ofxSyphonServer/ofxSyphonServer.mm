...
void ofxSyphonServer::publishScreen()
{
	if(bSetup)
    {
		...
		[mSyphon publishFrameTexture:texData.textureID textureTarget:texData.textureTarget imageRegion:NSMakeRect(0, 0, width, height) textureDimensions:NSMakeSize(width, height) flipped:NO];
...
void ofxSyphonServer::publishTexture(ofTexture* inputTexture)
{
	...
	if(bSetup && inputTexture->bAllocated())
    {
		...
		[mSyphon publishFrameTexture:texData.textureID textureTarget:texData.textureTarget imageRegion:NSMakeRect(0, 0, texData.width, texData.height) textureDimensions:NSMakeSize(texData.width, texData.height) flipped:texData.bFlipTexture];
