...
ofxIpVideoGrabber::ofxIpVideoGrabber() : ofBaseVideo(), ofThread() {
    ...
    ofImage* img0 = new ofImage();
    img0->setUseTexture(false);

    ofImage* img1 = new ofImage();
    img1->setUseTexture(false);

    image.push_back(img0);
    ...
    ///ofAddListener(ofEvents.exit, this, &ofxIpVideoGrabber::exit);  // THIS IS A BAD IDEA!
...
//--------------------------------------------------------------
void ofxIpVideoGrabber::setName(string _name) {
    cameraName = _name;
}
        ...
        if(c > 0) {
            if(mode == MODE_HEADER && cBuf[c-1] == '\r' && cBuf[c] == '\n') {
                ...
                if(cBuf[c] == SOI ) {
                    ...
                    lock(); 
                    {
                      //  cout <<"ABOUT THE LOAD AN IMAGE " << isThreadRunning() << " " << uri.toString() << endl;
...
//--------------------------------------------------------------
void ofxIpVideoGrabber::exit() {
    ofLog(OF_LOG_NOTICE, "ofxIpVideoGrabber: Closing down connection to : " + getURI());
...
ofTexture& ofxIpVideoGrabber::getTextureReference() {
    isNewFrame = false;// TODO ... hmmmm
    return image[ci]->getTextureReference();
}
...
void ofxIpVideoGrabber::update() {
    ...
    if(threadRunning && connected && isNewFrameLoaded) {
